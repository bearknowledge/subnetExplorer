...
export async function getServerSideProps() {
  try {
    ...
    const averageGas = await db.collection("transactions").aggregate([{$group: {_id:null, avg_val:{$avg:"$gasPrice"}}}]).toArray()
  ...
  return (
    ...
    <div className="flex min-h-screen flex-col mobile:p-4 laptop:p-12">
     ...
     <div className="flex flex-row mobile:items-center">
      ...
      <h1> {utils.formatEther((String(Math.floor(blocks.averageGas[0].avg_val))))}</h1>
      </span>
      </div>
      </div>
      </div>
